    ...
    **************************************/ 
    ...
    while (packet = pcap_next(handle, &header))
    {
        ...
        if(ip_hdr->ip_p == IPPROTO_TCP) {
            ...
            if(size_tcp > 20 & tcp_hdr->op_kind == 8) {
            // if(size_tcp > 20 & tcp_hdr->op_kind > 3) {
                ...
                printf("%u %u %u %u %d %d %d %d %d %d %d %d %u %u %d %u %u\n", 
                    ...
                    ntohl(tcp_hdr->op_tsval), ntohl(tcp_hdr->op_tsecr));

                if(DEBUG1) {
                    printf("=> src_port=%u, dst_port=%u, seq=%u, ack=%u, fin=%d, syn=%d, rst=%d, push=%d, ack=%d, urg=%d, ece=%d, cwr=%d, win=%u, urp=%u, payload=%d, offset=%d, option_kind=%d, option_len=%u, ts_val=%u, ts_reply=%u\n", 
                        ntohs(tcp_hdr->th_sport), ntohs(tcp_hdr->th_dport), 
                        ntohl(tcp_hdr->th_seq), ntohl(tcp_hdr->th_ack), 
                        is_fin, is_syn, is_rst, is_push, is_ack, is_urg, is_ece, is_cwr,
                        ntohs(tcp_hdr->th_win), ntohs(tcp_hdr->th_urp), payload_size, size_tcp,
                        tcp_hdr->op_kind, tcp_hdr->op_len, ntohl(tcp_hdr->op_tsval), ntohl(tcp_hdr->op_tsecr));

                    // printf("========= %d, %d, %d, %d, %d, %d\n", *((char *)tcp_hdr + 20), *((char *)tcp_hdr + 21), *((char *)tcp_hdr + 22), *((char *)tcp_hdr + 23), *((char *)tcp_hdr + 24), *((char *)tcp_hdr + 25));
                    // printf("check size=%lu, %lu\n", sizeof(u_int), sizeof(tcp_hdr->op_tsval));
                }

            }   // END of TCP timestamp option
