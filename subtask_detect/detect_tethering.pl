...
##   2. "src" or "dst"
##      only consider the specific sources or destionations
##   3. src/dst IP address
##      the specific address
##    
##
##  e.g.
##    perl detect_tethering.pl ../data/testbed/tcp_traces/text5/2013.07.11.HTC.iperf.2min.pcap.txt "src" "10.0.2.5"
##
##########################################
...
if(@ARGV >= 3) {
    if($ARGV[1] eq "src") {
        $FIX_SRC = 1;
        $FIX_SRC_ADDR = $ARGV[2];
        print "  - fix src: $FIX_SRC_ADDR\n" if($DEBUG2);
    }
    elsif($ARGV[1] eq "dst") {
        $FIX_DST = 1;
        $FIX_DST_ADDR = $ARGV[2];
        print "  - fix dst: $FIX_DST_ADDR\n" if($DEBUG2);
    }
