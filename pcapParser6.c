...
//
// pcapParser6
//   copied form pcapParser5.c
//
//  reads in a pcap file and find TCP option: window scale
//
//  e.g.
//      ./pcapParser6 /data/ychen/sprint/pcap/omni.out.49.eth.pcap
//
// ---------------------------------------------------------
            ...
            **************************************/
            ...
            while(size_tcp - parsed_option_size > 20) {
                ...
                if(tcp_kind == 1) {
                    ...
                    u_char tcp_ws = tcp_option_ptr[2];

                    parsed_option_size += 3;

                    if(DEBUG4) {
                        printf("TCP option 3: Window Scale\n");

                        if(tcp_op_len != 3) {
                            printf("wrong format of WSOPT\n");
                        }
                        printf("pkt %d\n", cnt);
                    }
                    ...
                    // --------------------------
                    // tcp format:
                    //  <src port> <dst port> <seq> <ack seq> <flag fin> <flag syn> <flag rst> <flag push> <flag ack> <flag urg> <flag ece> <flag cwr> <win> <urp> <payload len> <window scale>
                    printf("%u %u %u %u %d %d %d %d %d %d %d %d %u %u %d %u\n",
                    ...
                    if(DEBUG2) {
                        ...
                        printf("=> src_port=%u, dst_port=%u, seq=%u, ack=%u, fin=%d, syn=%d, rst=%d, push=%d, ack=%d, urg=%d, ece=%d, cwr=%d, win=%u, urp=%u, payload=%d\n",
                            ...
                            is_fin, is_syn, is_rst, is_push, is_ack, is_urg, is_ece, is_cwr,
                            ntohs(tcp_hdr->th_win), ntohs(tcp_hdr->th_urp), payload_size);
                        printf("=> tcp_hdr_size=%u, tcp_option_kind=%u, option_len=%u, win_scale=%u\n",
