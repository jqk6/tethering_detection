...
my $DEBUG3 = 1; ## fix the clock frequency of UT machines to 250Hz
...
my @freq_candidates = (250, 1000);  ## choose the clock frequency as the closest one
my $freq_threshold = 0.4;           ## the threshold if close to one of the above frequency
my $threshold = 100;                ## only calculate IPs with enough TCP packets
...
foreach my $this_ip (keys %{ $ip_info{IP} }) {
    ...
    foreach my $this_tx_time (sort {$a <=> $b} (keys %{ $ip_info{IP}{$this_ip}{TX_TIME} })) {
        ...
        if($freq == 0) {
            ...
            print "frequency of $this_ip = $freq\n" if($DEBUG2);
        }
        ...
        print FH "$this_tx_time, $this_rx_time, $x, $v, $w, $diff\n";
    }
    close FH;
