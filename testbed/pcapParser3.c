...
//
//  e.g. 
//      ./pcapParser3 /export/home/ychen/testbed/exp2/pcap/2013.06.24.AP.pcap
//      
// ---------------------------------------------------------
    ...
    **************************************/ 
    ...
    while (packet = pcap_next(handle, &header))
    {
        ...
        if(ip_hdr->ip_p == IPPROTO_TCP) {
            ...
            if(payload_size == 0) {
                // we want to look into HTTP headers, so the payload should not be 0

                // ----------
                // DEBUG
                // ----------
                if(DEBUG1) {
                    printf("skip because the payload size is 0\n");
                }
            ...
            // HTTP
            // -------------
            char *tcp_payload = (char *)((char *)tcp_hdr + size_tcp);
            char *tcp_payload_cpy = (char *)malloc(payload_size + 1);
            memcpy(tcp_payload_cpy, tcp_payload, payload_size);
            tcp_payload_cpy[payload_size] = '\0';


            char * http_ind = strstr(tcp_payload_cpy, "HTTP");
            char * http_end_ind = strstr(tcp_payload_cpy, "\r\n\r\n");

            if(http_ind == NULL) {
                if(DEBUG1) {
                    printf("skip because of we cannot HTTP substring\n");
                }
            ...
            int http_size = (int)(http_end_ind - tcp_payload_cpy);
