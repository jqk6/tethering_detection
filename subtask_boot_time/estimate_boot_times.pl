...
## 2013/07/18 @ Narus
##
## Estimate the boot times of a set of machines
##
## - input: 
##     a) parsed_pcap_text
##     format
...
foreach my $this_ip (keys %{ $ip_info{IP} }) {
    ...
    next if($FIX_SRC and !($this_ip =~ /$FIX_SRC_ADDR/));
    print "  $this_ip: ".scalar(keys %{ $ip_info{IP}{$this_ip}{TX_TIME} })."\n" if($DEBUG2);


    MyBootTime::estimate_boot_times(\%{ $ip_info{IP}{$this_ip} });
}
