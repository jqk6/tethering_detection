...
##
##  e.g.
##      perl analyze_sprint_text.pl /data/ychen/sprint/text/omni.out.49.eth.pcap.txt
##################################################
...
open FH, "> $output_dir/file.$file_id.pkt.ts.txt" or die $!;
print FH join(", ", @pkt_ts);
close FH;

## individual
open FH_ID, "> $output_dir/file.$file_id.ids.ts.txt" or die $!;
open FH_TTL, "> $output_dir/file.$file_id.ttl.txt" or die $!;
open FH_TTL_TS, "> $output_dir/file.$file_id.ttl.ts.txt" or die $!;
open FH_TPUT, "> $output_dir/file.$file_id.tput.ts.txt" or die $!;
open FH_PKT, "> $output_dir/file.$file_id.pkt.ts.txt" or die $!;
open FH_ENT, "> $output_dir/file.$file_id.len_entropy.ts.txt" or die $!;
foreach my $this_ip_pair (keys %ip_info) {
