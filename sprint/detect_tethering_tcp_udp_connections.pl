...
## 2013/06/24 @ Narus 
##
## Read in results from "analyze_sprint_tcp_udp_connections.pl" and detect tethering using number of connections.
## e.g. > n connections at any time using time bin size b
##
## - input: file_id
##     The file ID of 3-hr Sprint Mobile Dataset.
##     This program uses this ID to look up the output files from "analyze_sprint_tcp_udp_connections.pl", i.e.
##     ./output/
...
##          ./tethered_clients/TCP_UDP_Connections_timebin<time bin size>.threshold<threshold>.<file id>.txt
##
##  e.g.
##      perl detect_tethering_tcp_udp_connections.pl 49
##
##################################################
...
foreach my $this_timebin (@timebins) {
    my $file_name = "file.$file_id.tcp_udp_connections.bin$this_timebin.txt";
...
foreach my $this_timebin (@timebins) {
    foreach my $this_threshold (@thresholds) {
        my $file_output = "TCP_UDP_Connections_timebin$this_timebin.threshold$this_threshold.$file_id.txt";
