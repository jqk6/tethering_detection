...
##
##  e.g.
##    perl statistics.sigcomm08.v2.pl sigcomm08.filter.dup1.host0.2.bt0.s1
##
##########################################
...
# Variables
#############
my $input_dir  = "../processed_data/subtask_parse_sigcomm08/gen_trace";
my $output_dir = "../processed_data/subtask_parse_sigcomm08/statistics";
my $output_prob_dir = "../processed_data/subtask_parse_sigcomm08/prob";
my $output_dect_dir = "../processed_data/subtask_parse_sigcomm08/detection";
...
foreach my $this_ip (keys %{ $ip_info{SRC} }) {
    ...
    ########################################
    my $ip_ts_mono_disorder_thresh = 0.00001;
    if($ratio_disorder < $ip_ts_mono_disorder_thresh) {
    ...
    ########################################
    my $ip_ts_mono_gap_thresh = 2;
    if($num_large_gap < $ip_ts_mono_gap_thresh) {
    ...
    # print "    > $this_ip: freq stab: freq=$this_freq, stdev=$freq_stdev\n";
    ...
    my $ip_freq_stdev_thresh = 21;
    if($freq_stdev > 0 and $freq_stdev < $ip_freq_stdev_thresh) {
        $out_info{IP}{$this_ip}{"freq stdev < 100"} = 1;
        $out_info{IP}{$this_ip}{"freq stdev >= 100"} = 0;
        $out_info{IP}{$this_ip}{"freq stdev:N/A"} = 0;
    }
    elsif($freq_stdev >= $ip_freq_stdev_thresh) {
        $out_info{IP}{$this_ip}{"freq stdev < 100"} = 0;
        $out_info{IP}{$this_ip}{"freq stdev >= 100"} = 1;
        $out_info{IP}{$this_ip}{"freq stdev:N/A"} = 0;
    }
    ...
    ##############################
    my $ip_bt_stdev_thresh = 4600;
    if($bt_stdev > 0 and $bt_stdev < $ip_bt_stdev_thresh) {
    ...
    elsif($bt_stdev >= $ip_bt_stdev_thresh) {
        $out_info{IP}{$this_ip}{"boot time stdev < 500"} = 0;
        $out_info{IP}{$this_ip}{"boot time stdev >= 500"} = 1;
        $out_info{IP}{$this_ip}{"boot time stdev:N/A"} = 0;
    }
...
foreach my $this_os (keys %{ $stat_info{OS} }) {
    ...
    if(exists $stat_info{OS}{$this_os}{FREQ}{VALUE} ) {
        @dev_freqs = sort {$a <=> $b} @{ $stat_info{OS}{$this_os}{FREQ}{VALUE} };
        print "".join(" ", @dev_freqs)."\n";
        print FH join("\n", @dev_freqs);
    }
    close FH;
}
