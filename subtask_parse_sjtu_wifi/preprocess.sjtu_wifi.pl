...
my $NUM_PKT_PER_FLOW = 10;
my $FLOW_LEN = 10; ## in seconds
...
if(-e "$output_dir/$filename.filter.txt.bz2") {
    my $cmd = "rm \"$output_dir/$filename.filter.txt.bz2\"";
    `$cmd`;
}
...
open FH_OUT, "> $output_dir/$filename.filter.txt" or die $!;
...
my $cmd = "bzip2 \"$output_dir/$filename.filter.txt\"";
`$cmd`;
