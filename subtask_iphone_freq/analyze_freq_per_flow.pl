...
##
##  e.g.
##      perl analyze_freq_per_flow.pl ../data/testbed/tcp_traces/text5/2013.10.14.iphone.tr1.iperf.pcap.txt 10000 0.8
##################################################
...
my $FIX_DEST_ADDR = "192.168.1.7|192.168.1.3|10.0.2.|192.168.0.|128.83.120.167";
...
my $FIX_SRC_ADDR  = "192.168.1.7|192.168.1.3|10.0.2.|192.168.0.|128.83.120.167";
...
my $PLOT_IP       = "192.168.1.7|192.168.1.3|10.0.2.|192.168.0.|128.83.120.167";
...
my $gnuplot_freq_file      = "plot_freq_per_flow";
my $gnuplot_ts_file        = "plot_ts_per_flow";
...
foreach my $this_ip (keys %{ $ip_info{IP} }) {
    foreach my $this_conn (keys %{ $ip_info{IP}{$this_ip}{CONN} } ) {
        ...
        my $cnt = 0;
        my $sum_freq = 0;
        $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} = -1;
        foreach my $ind (0 .. scalar(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{TX_TIME} })-1) {
            ...
            if($this_rx_time - $first_rx_time < $SEG_INI_SEC) {
                next;
            }
            $cnt ++;

            if($cnt % $seg_len == 0) {
                push(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{AVG_FREQS} }, $sum_freq / $seg_len);
                $first_tx_time = $this_tx_time;
                $first_rx_time = $this_rx_time;
                $sum_freq = 0;
                $cnt = 0;
            ...
            if($DEBUG0 and ($this_rx_time == $first_rx_time or $this_tx_time == $first_tx_time)) {
                print "first ($first_rx_time, $first_tx_time), latest ($this_rx_time, $this_tx_time)\n" if($DEBUG2);
                next;
            }
            ...
            $sum_freq += $this_freq;
            if($this_boot_time < $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} or 
               $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} < 0) {
                $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} = $this_boot_time;
            }
            ...
            push(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{EWMA_BS} }, $ewma_boot_time);
            if($ewma_boot_time < $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} or 
               $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} < 0) {
                $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS} = $ewma_boot_time;
            }

        }  ## end for each packet
        if($cnt != 0) {
            push(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{AVG_FREQS} }, $sum_freq / $cnt);
        }
    }  ## end for each conn
}  ## end for each ip
...
foreach my $this_ip (keys %{ $ip_info{IP} }) {
    foreach my $this_conn (keys %{ $ip_info{IP}{$this_ip}{CONN} }) {
        ...
        open FH, ">$output_dir/$pure_name.$this_ip.conn$this_conn.seg$seg_len.freq_ts.txt" or die $!;
        my $min_freq = 1000;
        my $max_freq = 0;
        ...
        foreach my $ind (0 .. scalar(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{FREQS} })-1) {
            ...
            ## avg frequency
            my $avg_ind = floor($ind / $seg_len);
            if(exists $ip_info{IP}{$this_ip}{CONN}{$this_conn}{AVG_FREQS}[$avg_ind]) {
                print FH $ip_info{IP}{$this_ip}{CONN}{$this_conn}{AVG_FREQS}[$avg_ind].", ";
            }
            else {
                print FH "0, ";
            }
            ...
            ## boot time
            print FH ($ip_info{IP}{$this_ip}{CONN}{$this_conn}{BOOT_TIME}[$ind] - $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS}).", ";

            ## ewma frequency
            print FH $ip_info{IP}{$this_ip}{CONN}{$this_conn}{EWMA_FREQS}[$ind].", ";

            ## ewma boot time
            print FH ($ip_info{IP}{$this_ip}{CONN}{$this_conn}{EWMA_BS}[$ind] - $ip_info{IP}{$this_ip}{CONN}{$this_conn}{MIN_BS})."\n";
        }
        close FH;
        ...
        if($this_ip =~ /$PLOT_IP/) {
            ...
            my $cmd = "sed 's/DATA_DIR/output_freq/g; s/FIG_DIR/figures_freq/g; s/X_LABEL/time/g; s/Y_LABEL/frequency/g; s/FILE_NAME/$pure_name.$this_ip.conn$this_conn.seg$seg_len.freq_ts/g; s/FIG_NAME/$pure_name.$this_ip.conn$this_conn.seg$seg_len.freq_ts/g; s/DEGREE/-45/g; s/X_RANGE_S//g; s/X_RANGE_E//g; s/Y_RANGE_S/".max(0, $min_freq-100)."/g; s/Y_RANGE_E/".min(1000, $max_freq+100)."/g;' $gnuplot_freq_file.plot.mother > tmp.$gnuplot_freq_file.$pure_name.$this_ip.conn$this_conn.plot";
            `$cmd`;
            ...
            my $cmd = "sed 's/DATA_DIR/output_freq/g; s/FIG_DIR/figures_freq/g; s/X_LABEL/time/g; s/Y_LABEL/boot time/g; s/FILE_NAME/$pure_name.$this_ip.conn$this_conn.seg$seg_len.freq_ts/g; s/FIG_NAME/$pure_name.$this_ip.conn$this_conn.seg$seg_len.bt/g; s/DEGREE/-45/g; s/X_RANGE_S//g; s/X_RANGE_E//g; s/Y_RANGE_S//g; s/Y_RANGE_E//g; s/DATA_IND_1/4/g; s/DATA_IND_2/6/g;' $gnuplot_boot_time_file.plot.mother > tmp.$gnuplot_boot_time_file.$pure_name.$this_ip.conn$this_conn.plot";
            `$cmd`;
            $cmd = "gnuplot tmp.$gnuplot_boot_time_file.$pure_name.$this_ip.conn$this_conn.plot";
            `$cmd`;

            print "$pure_name, $this_ip, $this_conn, $seg_len, $ewma_alpha, seg, ".MyUtil::stdev(\@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{BOOT_TIME} }).", ewma, ".MyUtil::stdev(\@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{EWMA_BS} })."\n";
        }
    }
}
...
#####
## Generate ts output
print STDERR "start to generate ts output..\n" if($DEBUG2);

foreach my $this_ip (keys %{ $ip_info{IP} }) {
    foreach my $this_conn (keys %{ $ip_info{IP}{$this_ip}{CONN} }) {

        open FH, ">$output_dir/$pure_name.$this_ip.conn$this_conn.ts.txt" or die $!;
        foreach my $ind (0 .. scalar(@{ $ip_info{IP}{$this_ip}{CONN}{$this_conn}{TX_TIME} })-1) {
            print FH ($ip_info{IP}{$this_ip}{CONN}{$this_conn}{RX_TIME}[$ind]-$ip_info{IP}{$this_ip}{CONN}{$this_conn}{RX_TIME}[0]).", ".($ip_info{IP}{$this_ip}{CONN}{$this_conn}{TX_TIME}[$ind]-$ip_info{IP}{$this_ip}{CONN}{$this_conn}{TX_TIME}[0])."\n";
        }
        close FH;
        ...
        ## gnuplot
        if($this_ip =~ /$PLOT_IP/) {
            print STDERR "  plot: $pure_name.$this_ip.conn$this_conn ..\n" if($DEBUG2);
            my $cmd = "sed 's/DATA_DIR/output_freq/g; s/FIG_DIR/figures_freq/g; s/X_LABEL/time/g; s/Y_LABEL/Timestamp/g; s/FILE_NAME/$pure_name.$this_ip.conn$this_conn.ts/g; s/FIG_NAME/$pure_name.$this_ip.conn$this_conn.ts/g; s/DEGREE/-45/g; s/X_RANGE_S//g; s/X_RANGE_E//g; s/Y_RANGE_S//g; s/Y_RANGE_E//g; s/TITLE_1/freq/g; s/TITLE_2/avg freq/g;' $gnuplot_ts_file.plot.mother > tmp.$gnuplot_ts_file.$pure_name.$this_ip.conn$this_conn.plot";
            `$cmd`;
            $cmd = "gnuplot tmp.$gnuplot_ts_file.$pure_name.$this_ip.conn$this_conn.plot";
            `$cmd`;
        }
    }
}
