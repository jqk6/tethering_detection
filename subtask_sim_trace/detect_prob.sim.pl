...
##
##  e.g.
##    perl detect_prob.sim.pl sim.i5.a5.w5.s1.dup1.host0.2.bt0.s1 sim.i5.a5.w5.s1.dup1.host0.2.bt0.s1
##
##########################################
...
my $input_dir_prob  = "../processed_data/subtask_sim_trace/statistics";
my $output_dir = "../processed_data/subtask_sim_trace/detect";
...
$tethering_features{"boot_time_first_last_stdev.thresh1000.diff"} = 1;
$tethering_features{"boot_time_first_last_stdev.thresh1000.same"} = 1;
...
$tethering_features{"ts_not_monotonic"} = 1;
$tethering_features{"ttl_num.gap_4==1"} = 1;
$tethering_features{"ttl_num.gap_4>1"} = 1;

my %os_features = ();
$os_features{OS}{"Windows"}{"freq_stable"} = 1;
$os_features{OS}{"Windows"}{"freq_unstable"} = 1;
$os_features{OS}{"Windows"}{"id_monotonic_flows"} = 1;
$os_features{OS}{"Windows"}{"id_monotonic_partial"} = 1;
$os_features{OS}{"Windows"}{"id_monotonic_pkts"} = 1;
$os_features{OS}{"Windows"}{"id_random"} = 1;
$os_features{OS}{"Windows"}{"tcp_ts_opt_high"} = 1;
$os_features{OS}{"Windows"}{"tcp_ts_opt_low"} = 1;

$os_features{OS}{"Apple"}{"freq_stable"} = 1;
$os_features{OS}{"Apple"}{"freq_unstable"} = 1;
$os_features{OS}{"Apple"}{"id_monotonic_flows"} = 1;
$os_features{OS}{"Apple"}{"id_monotonic_partial"} = 1;
$os_features{OS}{"Apple"}{"id_monotonic_pkts"} = 1;
$os_features{OS}{"Apple"}{"id_random"} = 1;
$os_features{OS}{"Apple"}{"tcp_ts_opt_high"} = 1;
$os_features{OS}{"Apple"}{"tcp_ts_opt_low"} = 1;

$os_features{OS}{"Android"}{"freq_stable"} = 1;
$os_features{OS}{"Android"}{"freq_unstable"} = 1;
$os_features{OS}{"Android"}{"id_monotonic_flows"} = 1;
$os_features{OS}{"Android"}{"id_monotonic_partial"} = 1;
$os_features{OS}{"Android"}{"id_monotonic_pkts"} = 1;
$os_features{OS}{"Android"}{"id_random"} = 1;
$os_features{OS}{"Android"}{"tcp_ts_opt_high"} = 1;
$os_features{OS}{"Android"}{"tcp_ts_opt_low"} = 1;
...
while(<FH_PR>) {
    ...
    foreach my $this_os (keys %{ $os_features{OS} }) {
        next if($type eq $this_os and !(exists $os_features{OS}{$this_os}{$feature}));
    }
    ...
    foreach my $this_ip (keys %{ $ip_info{SRC} }) {
        ...
        foreach my $this_f (sort {$a cmp $b} (keys %{ $ip_info{SRC}{$this_ip}{FEATURE} })) {
            if(exists $prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}) {
                $f_numerator *= ($prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F_TETHER});
                $f_denominator *= $prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F};


                ## copying the feature
                foreach my $ti (0 .. $num_dup-1) {
                    $f_numerator *= ($prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F_TETHER});
                    $f_denominator *= $prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F};
                }
        ...
        foreach my $this_os (keys %{ $os_features{OS} }) {
            ...
            my $os_denominator = 1;
            foreach my $this_f (sort {$a cmp $b} (keys %{ $ip_info{SRC}{$this_ip}{FEATURE} })) {
                if(exists $prob_info{TYPE}{$this_os}{FEATURE}{$this_f}) {
                    $os_numerator *= ($prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F_TETHER});
                    $os_denominator *= $prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F};
                }
                else {
                    ## the feature is not used
                    # print "  $this_f: XXX - no such feature\n" if($DEBUG5);
            ...
            my $os_prob = $os_numerator * $prob_info{TYPE}{$this_os}{PR_OS} / $os_denominator;
        }


        my $prob = $f_numerator * $PR_TETHER / $f_denominator;
        # my $prob = $f_numerator / $denominator;
        # print "  prob = $prob\n";
        print "$prob\n" if($DEBUG5);
...
if($DEBUG7) {
    ...
    foreach my $this_ip (keys %{ $ip_info{SRC} }) {
        ...
        ## Tethering feature
        print "  Tethering features:\n";
        my $f_numerator = 1;
        my $f_denominator = 1;
        foreach my $this_f (sort {$a cmp $b} (keys %{ $ip_info{SRC}{$this_ip}{FEATURE} })) {
            if(exists $prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}) {
                if($DEBUG7) {
                    print "    $this_f: Pr(f|tether)=".$prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F_TETHER}.", Pr(f)=".$prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F}.", Pr(tether|f)=".($prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F_TETHER}*$PR_TETHER/$prob_info{TYPE}{"tethering"}{FEATURE}{$this_f}{PR_F})."\n";
                }
        ...
        my $prob = $f_numerator * $PR_TETHER / $f_denominator;
        # my $prob = $f_numerator / $f_denominator;
        # print "  prob = $prob\n";
        ...
        foreach my $this_os (keys %{ $os_features{OS} }) {
            ...
            print "  $this_os features:\n";
            my $os_numerator = 1;
            my $os_denominator = 1;
            foreach my $this_f (sort {$a cmp $b} (keys %{ $ip_info{SRC}{$this_ip}{FEATURE} })) {
                if(exists $prob_info{TYPE}{$this_os}{FEATURE}{$this_f}) {
                    print "    $this_f: Pr(f|tether)=".$prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F_TETHER}.", Pr(f)=".$prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F}.", Pr(tether|f)=".($prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F_TETHER}*$prob_info{TYPE}{$this_os}{PR_OS}/$prob_info{TYPE}{$this_os}{FEATURE}{$this_f}{PR_F})."\n";
